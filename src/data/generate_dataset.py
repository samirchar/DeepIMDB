...
def get_metadata(file_path="data/raw/AllMoviesDetailsCleaned.csv", sep=";"):
    ...
    ## Filter rows ##
    meta_data["release_date"] = pd.to_datetime(
        meta_data["release_date"], errors="coerce"
    )

    # This can be changed to "made_in_us" filter instead of original language
    mask1 = (
        (meta_data["release_date"].dt.year >= 1980)
    ...
    ## Format data ##
    meta_data["imdb_id"] = pd.to_numeric(
        meta_data["imdb_id"].str.replace("tt", ""), errors="coerce"
    )
    ...
    ## Clean bad overviews ##
    meta_data[
        meta_data["overview"].apply(lambda x: len(x) if isinstance(x, str) else x) <= 2
    ] = np.nan
...
def get_imdb_dump_data(
    imdb_ids_list=processed_meta_data["imdb_id"].unique(),
    input_dir="data/raw/oficial_imdb_dataset",
):

    # READ DATA #
    names = pd.read_csv(
        os.path.join(input_dir, "name.basics.tsv"),
        sep="\t",
        usecols=["nconst", "primaryName"],
    )

    titles = pd.read_csv(
        os.path.join(input_dir, "title.basics.tsv"),
        sep="\t",
        usecols=["tconst", "titleType", "genres", "runtimeMinutes"],
    )

    principals = pd.read_csv(os.path.join(input_dir, "title.principals.tsv"), sep="\t")

    ratings = pd.read_csv(os.path.join(input_dir, "title.ratings.tsv"), sep="\t")
    ...
    principals.replace(
        {"actor": "cast", "actress": "cast", "self": "cast"}, inplace=True
    )

    ## Sort by credits order
    principals.sort_values(
        by=["imdb_id", "category", "ordering"], ascending=True, inplace=True
    )
    ...
    grouped_principals = (
        principals.groupby(["imdb_id", "category"])["primaryName"]
        .apply(lambda x: "|".join(x.iloc[:top_n].tolist()))
        .reset_index()
    )

    grouped_principals = (
        grouped_principals.pivot(
            columns="category", index="imdb_id", values="primaryName"
        )
        .reset_index()
    ...
    grouped_principals[categ] = grouped_principals[categ].apply(
        lambda x: x.str.lower().str.strip(), axis=1
    )
...
def merge_all(use_columns=USE_COLUMNS):
    ...
    poster_ratings = pd.read_csv(
        "data/processed/posters_links.csv"
    )  # TODO: This is unnecessary because i get it from API

    # Rename Columns
    box_office_mojo_data_v2.rename(
        columns={
            i: f"revenue_{i.lower()}_BOM" for i in box_office_mojo_data_v2.columns[:3]
        },
        inplace=True,
    )

    # MERGE #
    df = titles.drop(["director"], axis=1).merge(
        processed_meta_data.drop(["genres"], axis=1), how="left", on="imdb_id"
    )
    ...
    df["Budget"] = (
        df["Budget"]
        .apply(
            lambda x: float(Decimal(re.sub(r"[^\d.]", "", x)))
            if isinstance(x, str)
            else np.nan
        )
        .replace(0, np.nan)
...
def download_poster(
    row, data_path="data/processed", id_name="imdb_id", poster_col="cover url"
):
    ...
    file_path = os.path.join(data_path, f"posters/{int(row[id_name])}.jpg")

    status = download_image_url(row[poster_col], file_path)
    if status == "ERROR":
        return int(row[id_name])


def data_splits(
    input_dir="data/processed", output_dir="data/processed", train_proportion=0.7
):
    df = pd.read_csv(
        os.path.join(input_dir, "df.csv"),
        parse_dates=["release_date"],
        usecols=["imdb_id", "release_date"],
    )
    df = df.sort_values(by="release_date", ascending=True)

    train_val_split = int(round(len(df) * train_proportion))
    val_test_split = int(
        round(len(df) * (train_proportion + (1 - train_proportion) / 2))
    )
    ...
    df[df["imdb_id"].isin(train_ids)].to_csv(
        os.path.join(output_dir, "train.csv"), index=False
    )
    df[df["imdb_id"].isin(val_ids)].to_csv(
        os.path.join(output_dir, "val.csv"), index=False
    )
    df[df["imdb_id"].isin(test_ids)].to_csv(
...
if __name__ == "__main__":
    ...
    if args.download_posters == 1:
        ...
        errors = Parallel(n_jobs=-1)(
            delayed(download_poster)(row)
            for _, row in df[["imdb_id", "cover url"]].iterrows()
        )
